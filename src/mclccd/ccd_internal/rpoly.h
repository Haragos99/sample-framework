...
_10:
	 ...
	 */
	static void quadsd(int nn,double *u_, double *v_, double *p, double *q,
			double *a,double *b_)
	{
		double c_;
		int i;
	 ...
	 */
	static void quad(double a_, double b1, double c_, double *sr,double *si,
			double *lr,double *li)
	{
		double b_,d_,e_;
		...
		if (c_ == 0.0) {         /* one real root, one zero root */
			*sr = 0.0;
			*lr = -b1/a_;
			*si = 0.0;
			*li = 0.0;
