...
static inline bool is_2D( const std::vector<Eigen::Vector3d> &verts ){
	int nv = verts.size();
	for( int i=0; i<nv; ++i ){
		if( std::abs(verts[i][2])>0 ){ return false; }
	}
	return true;
}
...
double curr_t = isect.l;
/*
std::vector<Eigen::Vector3d> vt = {
	p0start*(1.0-curr_t) + p0end*curr_t,
	p1start*(1.0-curr_t) + p1end*curr_t,
	q0start*(1.0-curr_t) + q0end*curr_t,
	q1start*(1.0-curr_t) + q1end*curr_t
};
Eigen::Vector4d ee_barys;
double dist_t = Distance::edgeEdgeDistance(vt[0],vt[1],vt[2],vt[3],
	ee_barys[0], ee_barys[1], ee_barys[2], ee_barys[3]).norm();
if( dist_t > 10.0*eta ){ skip = true; }
*/
				if(!skip)
				{
					if(all_t){ all_t->emplace_back(curr_t); }
					mint = min(mint, curr_t);
					col = true;
				}
						}
					}
...
bool CTCD::vertexFaceCTCD(
		const Vector3d &q0start,
		...
		for (int j = 0; j < (int) e1.size(); j++)
		{
			for (int k = 0; k < (int) e2.size(); k++)
			{
				for (int l = 0; l < (int) e3.size(); l++)
				{
					...
					if(TimeInterval::overlap(intervals))
					{
						double curr_t = TimeInterval::intersect(intervals).l;
						if(all_t){ all_t->emplace_back(curr_t); }
						mint = std::min(curr_t, mint);
						col = true;
					}
				}
			}
...
bool CTCD::vertexEdgeCTCD(
					const Eigen::Vector2d &q0start,
	...
	auto pt_on_line = [&]( const Vector2d &p, const Vector2d &e0, const Vector2d &e1 ){
		double len = (e0-e1).norm();
		double p_e0 = (p-e0).norm();
		if( p_e0 > len ){ return false; }
		double p_e1 = (p-e1).norm();
		if( p_e1 > len ){ return false; }
		return true;
	};

	Vector2d u0 = q1start - q0start;
	Vector2d v0 = q2start - q0start;
	Vector2d u1 = q1end - q0end;
	Vector2d v1 = q2end - q0end;
	Vector2d up = u1-u0;
	Vector2d vp = v1-v0;

	//bool print_things = (int)t==-10;

	// Using signed triangle area
	// rel position u, v, rel velocity up, vp
	// det[ u + t*up, v + t*vp ] -->
	// det[u,v] + t ( det[u,vp] + det[up,v] ) + t^2 det[up,vp]
	Matrix2d Mc; Mc.col(0) = u0; Mc.col(1) = v0;
	double c = Mc.determinant();
	Matrix2d Mb0; Mb0.col(0) = u0; Mb0.col(1) = vp;
	Matrix2d Mb1; Mb1.col(0) = up; Mb1.col(1) = v0;
	double b = Mb0.determinant() + Mb1.determinant();
	Matrix2d Ma; Ma.col(0) = up; Ma.col(1) = vp;
	double a = Ma.determinant();
	double t0=2, t1=2;
	int nroots = getQuadRoots(a, b, c, t0, t1);
	//int nwrongside = 0;
	if( nroots == 0 ){ return false; }
	else
	{
		bool t0_on_line = pt_on_line(
			q0start*(1.0-t0) + q0end*t0,
			q1start*(1.0-t0) + q1end*t0,
			q2start*(1.0-t0) + q2end*t0
		);
		bool t1_on_line = pt_on_line(
			q0start*(1.0-t1) + q0end*t1,
			q1start*(1.0-t1) + q1end*t1,
			q2start*(1.0-t1) + q2end*t1
		);
		bool set_from_t0 = false;
		bool set_from_t1 = false;

		if( t0 >= 0 && t0 <= 1 && t0_on_line ){
			t = t0;
			set_from_t0 = true;
			if(all_t){ all_t->emplace_back(t0); }
		}
		if( t1 >= 0 && t1 <= 1 && t1_on_line ){
			t = (set_from_t0 ? t0 : t1);
			set_from_t1 = true;
			if(all_t){ all_t->emplace_back(t1); }
		}

		return (set_from_t0 || set_from_t1);
	}
...
bool CTCD::vertexVertexCTCD(const Vector3d &q1start,
   ...
   bool CTCD::vertexVertexCTCD(const Eigen::Vector2d &q1start,
							   const Eigen::Vector2d &q2start,
							   const Eigen::Vector2d &q1end,
							   const Eigen::Vector2d &q2end,
							   double eta, double &t,
								std::vector<double> *all_t ){
	// Vertices collide if dist^2 is less than squared sum of eta.
	// I.e. sphere-sphere collision.

	Vector2d v1 = q1end-q1start;
	Vector2d v2 = q2end-q2start;
	Vector2d s = q1start-q2start;
	Vector2d v = v1-v2; // rel velocity
	double r2 = eta + eta; // radius = eta, 

	// Overlap at t=0?
	double c = s.dot(s) - r2*r2;
	if( c < 0.0 ){
		t = 0.0;
		if(all_t){ all_t->emplace_back(0); }
		return true;
	}
